...
trap 'kill 0' EXIT INT TERM   # best-effort early cleanup; NOT sufficient on its own
wait
```

The `timeout` is what makes this safe — the `trap` is a courtesy that never runs if the shell is SIGKILLed. Prefer the smallest load and shortest deadline that reproduces the effect, and tell the user what you are about to start before you start it.
